...
if ($Data) {
    if (!$Data['email']) $Error['email'] = 'Email Boş Bırakılamaz';
    if (!$Data['pass']) $Error['pass'] = 'Şifre Boş Bırakılamaz';
    if (!$Error) {

        $Doctor = $Sql->Information("SELECT * FROM Doctor WHERE email='$Data[email]'");
        $Patient = $Sql->Information("SELECT * FROM Patient WHERE email='$Data[email]'");
